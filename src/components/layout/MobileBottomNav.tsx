...
export default function MobileBottomNav() {
  ...
  const isAdmin = session?.user?.email === 'chanmakara672@gmail.com'
  ...
  const menuLinks = [
    ...
    { href: '/purchase-history', label: locale === 'km' ? 'ប្រវត្តិទិញ' : 'Orders', icon: OrderHistoryIcon },
    { href: '/account', label: t('settings'), icon: AccountIcon },
    ...(isAdmin ? [{ href: '/admin/orders', label: t('adminPanel'), icon: AdminIcon }] : []),
    ...(isLoggedIn ? [] : [{ href: '/login', label: t('login'), icon: LoginIcon }]),
